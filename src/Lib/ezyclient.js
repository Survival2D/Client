//Bản EzyClient do TienNM5 sửa lại dựa trên EzyClient-v1.0.5 do vấn đề tương thích. Cocos dùng ES bản quá cũ rồi.
...
var EzyClient = function(config) {
    ...
    this.sendRequest = function(cmd, data) {
        if (this.unloggableCommands.indexOf(cmd) < 0) {
...
var EzyAppResponseHandler = function() {
    this.handle = function(data) {
        ...
        if (handler) handler(app, commandData); else EzyLogger.console("app: " + app.name + " has no handler for command: " + cmd);
    };
};
...
var EzyPluginResponseHandler = function() {
    this.handle = function(data) {
        ...
        if (handler) handler(plugin, commandData); else EzyLogger.console("plugin: " + plugin.name + " has no handler for command: " + cmd);
    };
};
...
var EzyEventMessageHandler = function(client) {
    ...
    this.handleEvent = function(event) {
        ...
        if (eventHandler) eventHandler.handle(event); else EzyLogger.console("has no handler with event: " + event.getType());
    };
    ...
    this.handleMessage = function(message) {
        ...
        if (this.unloggableCommands.indexOf(cmd) < 0) EzyLogger.console("received cmd: " + cmd.name + ", data: " + JSON.stringify(data));
        if (cmd === EzyCommand.DISCONNECT) this.handleDisconnectionData(data); else this.handleResponseData(cmd, data);
    };
    ...
    this.handleResponseData = function(cmd, responseData) {
        ...
        if (handler) handler.handle(responseData); else EzyLogger.console("has no handler with command: " + cmd.name);
    };
};
...
var EzyAppManager = function(zoneName) {
    ...
    this.getApp = function() {
        var app = null;
        if (this.appList.length > 0) app = this.appList[0]; else EzyLogger.console("has no app in zone: " + this.zoneName);
        return app;
    };
...
var EzyPluginManager = function(zoneName) {
    ...
    this.getPlugin = function() {
        var plugin = null;
        if (this.pluginList.length > 0) plugin = this.pluginList[0]; else EzyLogger.console("has no plugin in zone: " + this.zoneName);
        return plugin;
    };
