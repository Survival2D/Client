...
import {getDeviceId} from "react-native-device-info/src";
...
class NakamaManager extends cc.Node {
    ...
    loginWithDevice() {
        this.client = new Client(this.connectionData.scheme, this.connectionData.host, this.connectionData.port);
...
// If the user's device ID is already stored, grab that - alternatively get the System's unique device identifier.
        ...
        this.loginAsync(this.connectionData, this.client.authenticateDevice(SystemInfo.deviceUniqueIdentifier));
    }
    ...
    loginAsync(connectionData, sessionTask: () => Session): NakamaConnectionData {
        ...
        try {
            ...
            this.dispatchEvent(new cc.Event.EventCustom(GameEventType.LOGIN_SUCCESS, true));
        } catch (exception) {
            console.error(exception);
            this.dispatchEvent(new cc.Event.EventCustom(GameEventType.LOGIN_FAIL, true));
        }
    }
    ...
    disconnected() {
        this.dispatchEvent(new cc.Event.EventCustom(GameEventType.DISCONNECTED, true));
    }

    sendRPC(rpc: string, payload: object): RpcResponse {
